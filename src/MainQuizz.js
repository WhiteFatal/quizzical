    ...
    function renderQuestions () {                 
        return props.data.map((questionInstance, indQ) => {          
            return (   
                <QuizzQuestions 
                    id={`Q${indQ}`}
                    question={questionInstance.question.replace(/&#039;/g, "\'").replace(/&quot;/g, '\"').replace(/&ldquo;/g, "\“").replace(/&rsquo;/g, "\’").replace(/&rdquo;/g, "\”").replace(/&hellip;/g, "\…").replace(/&shy;/g, "\-").replace(/&amp;/g, "\&").replace(/&deg;/g, "\°")}
